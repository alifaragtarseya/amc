...
@section('content')
    @component('dashboard.layouts.includes.card', ['title' => $title])
        @slot('content')
            <form class="form" action="{{ $resource->id ? route('admin.package.update', $resource->id) : route('admin.package.store') }}"
                ...
                <div class="row">
                    ...
                    <div class="col-6  ">
                        ...
                        {!! Form::label('category_id', __('lang.category') , ['class' => 'form-label'] ) !!} <br>
                        {!! Form::select('category_id', $categories, old('category_id', $resource->category_id), ['class' => 'form-control select2', 'id' => 'category_id', 'placeholder' => __('lang.select')]) !!}
                    </div>
                ...
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="col-md-6 pt-3">
                        <div class="form-group">
                            ...
                            {!! Form::text("{$locale}[short_description]", old("{$locale}[short_description]", optional($resource->translate($locale))->short_description), ['class' => 'form-control ']) !!}
                        </div>
                    </div>
                @endforeach
