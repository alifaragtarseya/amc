...
class SliderController extends Controller
{
    ...
    public function store(StoreSlider $request)
    {
        ...
        if (isset($inputs['bg_image'])) {
            $inputs['bg_image'] = uploadImage( $inputs['bg_image'] , config('path.SLIDER_PATH') );
        }
        // dd($inputs);
    ...
    public function update(StoreSlider $request, $id)
    {
        ...
        $resource = $this->model->findOrFail($id);
        if (isset($inputs['image'])) {
            $inputs['image'] = uploadImage($inputs['image'], config('path.SLIDER_PATH'),null, $resource->image);
        }
        if (isset($inputs['bg_image'])) {
            $inputs['bg_image'] = uploadImage($inputs['bg_image'], config('path.SLIDER_PATH'),null, $resource->bg_image);
        }
        // dd($inputs);
